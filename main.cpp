...
void CalcMaquina(){
    ...
    if(action < 40){ //anda
        if(action < 30){
            colision = boneco2P.Colisao(timeDiference,pra_frente,*boneco1P.ObtemXadress(),*boneco1P.ObtemYadress());
            if(!colision)
            boneco2P.Move(timeDiference,pra_frente,*boneco1P.ObtemXadress(),*boneco1P.ObtemYadress());
        }
        else{
            colision = boneco1P.Colisao(timeDiference,pra_tras,*boneco2P.ObtemXadress(),*boneco2P.ObtemYadress());
            if(!colision)
            boneco2P.Move(timeDiference,pra_tras,*boneco2P.ObtemXadress(),*boneco2P.ObtemYadress());
        }

    }
    if(action > 60){//redireciona
...
//Atualiza o tempo do ultimo frame ocorrido
    ...
    if(keyStatus[(int)('w')])
    {
        colision = boneco1P.Colisao(timeDiference,pra_frente,*boneco2P.ObtemXadress(),*boneco2P.ObtemYadress());
        if(!colision)
