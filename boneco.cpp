...
void Boneco::DesenhaBoneco(GLfloat x, GLfloat y,GLfloat raio, GLfloat gThetaRight, GLfloat gThetaLeft, GLfloat gTheta2, GLfloat gTheta3, GLfloat bodyTheta, GLfloat R, GLfloat G, GLfloat B){
    ...
    DrawHead(raio, R,  G , B);
       if(isShowingCircle()){
        DesenhaColisaoCirc(*(obtemRaio())*2.8,1,1,1);
    }
...
void Boneco::Move(GLdouble time, Direction direction, GLfloat inimigoX, GLfloat inimigoY){
    float x=0,y=0;
    bool colisaoX = ColisaoX(time,direction,inimigoX,inimigoY);
    bool colisaoY = ColisaoY(time,direction,inimigoX,inimigoY);
    float vel = *ObtemVel();
...
bool Boneco::Colisao(GLdouble time, Direction direction, GLfloat inimigoX, GLfloat inimigoY){
    ...
    float colisionRadius = *(obtemRaio())*2.8;
...
bool Boneco::ColisaoX(GLdouble time, Direction direction, GLfloat inimigoX, GLfloat inimigoY){
    float x;
    x= (*ObtemVel())*cos((*(obtemAngulo()) +90)*M_PI/180)*time;
    if(direction==pra_tras){
        x=-x;
    ...
    float colisionRadius = *(obtemRaio())*2.8;
    float distancia = sqrt(quadrado((*ObtemXadress())+x-inimigoX) + quadrado(*(ObtemYadress()) - inimigoY));
    //printf("%f distancia, %f x, %f y\n", distancia,x,y);
    if(*(obtemRaio()) + colisionRadius > distancia){
        return true;
    }
...
bool Boneco::ColisaoY(GLdouble time, Direction direction, GLfloat inimigoX, GLfloat inimigoY){
    float y;
    y= (*ObtemVel())*sin((*(obtemAngulo()) + 90)*M_PI/180)*time;
    if(direction==pra_tras){
        y=-y;
    ...
    float colisionRadius = *(obtemRaio())*2.8;
    float distancia = sqrt(quadrado((*ObtemXadress())-inimigoX) + quadrado(*(ObtemYadress()) +y - inimigoY));
    //printf("%f distancia, %f x, %f y\n", distancia,x,y);
    if(*(obtemRaio()) + colisionRadius > distancia){
        return true;
    }
